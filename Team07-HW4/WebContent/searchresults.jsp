...
<body>
	...
	<div id="dialog-form">
		<form id="form1" name="form1" method="post" action="<c:url value='/SearchResults' />">
			<center>
				...
				<%
					if (flights.size() > 0) {
							session.setAttribute("Flightsearchdate", flights.get(0)
									.getDot());
				%>
				...
				<table width="788" border="1" align="center" bgcolor="#FAFAFA">
					...
					<c:forEach var="name" items="${flightResult}">
						<tr>
							<td><center>
									<input name="selectedflight" type="radio"
										value="${name.operator}${name.flightnumber}" />
								</center></td>
							<td><center>${name.operator} ${name.flightnumber}</center></td>
							<td><center>${name.dot}</center></td>
							<td><center>${name.source}</center></td>
							<td><center>${name.destination}</center></td>
							<td><center>${name.departure}</center></td>
							<td><center>${name.arrival}</center></td>
							<td><center>${name.stops}</center></td>
							<td><center>$ ${name.cost}</center></td>
						</tr>
					</c:forEach>
				</table>
